...
"""For full discloure, this code is heavily inspired by some projects I found online. The sources wont be sited since 
it solves the task... """
...
class YoutubeClassifier:
    ...
    def cleaning(self, titles=None, descriptions=None):
        ...
        for title, description in zip(titles, descriptions):
            ...
            # Descriptions usually contain links and generally more noise than titles...
            description = re.sub(r"http\S+|www\S+|https\S+", '', description, flags=re.MULTILINE)
            description = re.sub(r'\@\w+|\#', '', description)

            # Tokenization, stopword removal
            title_tokens = word_tokenize(title)
            filtered_titles = [w for w in title_tokens if w not in stop_words]

            description_tokens = word_tokenize(description)
            filtered_descriptions = [w for w in description_tokens if w not in stop_words]

            # Normalizing
            lemmatizer = WordNetLemmatizer()

            title = [lemmatizer.lemmatize(w, pos='a') for w in filtered_titles]
            lemma_title= " ".join(title)
            clean_titles.append(lemma_title)

            description = [lemmatizer.lemmatize(w, pos='a') for w in filtered_descriptions]
            lemma_description = " ".join(description)
            clean_descriptions.append(lemma_description)

        titles, descriptions = clean_titles, clean_descriptions
    ...
    def inspectFeatures(self):
        ...
        for current_class in list(self.labelEncoder.classes_):
            ...
            features_chi2 = chi2(self.features_title, labels == current_class_id)
        ...
        for current_class in list(self.labelEncoder.classes_):
            current_class_id = self.labelEncoder.transform([current_class])[0]
            features_chi2 = chi2(self.features_description, labels == current_class_id)
            indices = np.argsort(features_chi2[0])
            feature_names = np.array(
    ...
    def main(self):
        ...
        print("Successfully encoded labels.")
...
if __name__ == "__main__":
    ...
    titles = data['title'].tolist()[:1000]
    descriptions = data['description'].tolist()[:1000]
    categories = data['category'].tolist()[:1000]
