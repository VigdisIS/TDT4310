...
def Task4a():
    ...
    print("The probability of a Noun(NN) being 'we' is",
          tagwords["NN"].prob("we"))
    print("The probability of a Noun(VB) being 'like' is",
...
def Task4b():
    tagwords, tags = probDist(brown)
    result = tagwords["PP"].prob("I") * tags["PP"].prob("VB") * tagwords["VB"].prob("conduct") * tags["VB"].prob("PP")* tagwords["PP"].prob("my") * tags["PP"].prob("NN") * tagwords["NN"].prob("conduct")
    print(f"The probability of the tags PP VB PP NN used for the following sequence of words 'I conduct my conduct' is: {result}")
    return result
...
def Task4c():
    ...
    # YOUR CODE
    tagwords, tags = probDist(brown)
    dist_tags = set([tag for (tag, word) in get_tags(brown)])
    sentence = "You should invest in the stock market".split()
    result = ViterbiBestTag(dist_tags, tagwords, tags, sentence)
    print(result)
